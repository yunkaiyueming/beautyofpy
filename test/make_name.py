...
def make_name_test(xing, s1, s2):
        name = "%s%s%s" % (xing, s1, s2)
        rate_name2(name)
...
def rate_name(name, sex, y, m, d, h, i):
    ...
    if r.status_code == 200:
        try:
            ...
            suggest = suggest.replace("div", "")
            suggest = suggest.replace("script", "")
            suggest = suggest.replace("body", "")
            suggest = suggest.replace("html", "")
            suggest = suggest.replace("font", "")
            suggest = suggest.replace('style="clear:both"', "")
            suggest = suggest.replace("<", "")
            suggest = suggest.replace(">", "")
            suggest = suggest.replace('/', "")
            suggest = suggest.replace("src='bottom_cj.js'", "")
...
def rate_name2(name):
    rate_name(name, "男", "2018", "10", "15", "12", "00")

def main():
    print "%s,%s,%s,%s,%s,%s" % ("姓名","八字","笔画","数理","数理补充","建议")
    #make_name("刘", "世", SHUI)
    #make_name_test("蔡","瑀","桐")
    make_name1("蔡", "瑀", MU)
